...
# @@Created          :  Tuesday, Aug 13, 2024 19:12 EDT
# @@File             :  entrypoint.sh
...
SERVICE_USER="python"  # execute command as another user
SERVICE_GROUP="python" # Set the service group
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
if [ "$ENTRYPOINT_FIRST_RUN" != "no" ]; then
  ...
  if [ "$CONFIG_DIR_INITIALIZED" = "no" ] || [ "$DATA_DIR_INITIALIZED" = "no" ]; then
    [ "$ENTRYPOINT_MESSAGE" = "yes" ] && echo "Executing entrypoint script for python"
  fi
  # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
